...
object MixinImplementation {
  ...
  object BaseC extends Base with Value with Property with Selector with MixinDSL with Compiler
  val dsl: DSL = BaseC
  import dsl._
  ...
  /**
    ...
    *     background: white;
    */
  val simpleColorMixin : Seq[dsl.Declaration] = List( prop("color")      := value("red") ,
  ...
  /**
    ...
    *     margin: $padding;
    */
  val nestedMixin : (Seq[dsl.Declaration], String) => Seq[dsl.Declaration] =
    (mixin: Seq[dsl.Declaration], padding: String) => mixin :+ (prop("margin") := value(padding))
  ...
  /**
    ...
    *     width: $height * 2;
    */
   val doubledWidthMixin : Int => Seq[dsl.Declaration] = (height: Int) => List( prop("height") := value(height.toString) ,
